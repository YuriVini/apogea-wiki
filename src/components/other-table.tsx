...
export type OtherTableItem = OtherDatabaseType | Other;
...
export const OtherTable = ({
  ...
  title: string;
  items: OtherTableItem[];
  onDelete?: (itemId: string) => void;
}) => {
  const isMonsterType = items.length > 0 && items[0]?.type === "monster";
  const isDropCreatureType =
    items.length > 0 && items[0]?.type === "drop_creatures";
  ...
  function renderEditButton(item: OtherTableItem) {
    if (isAdmin) {
      return (
        <>
          <td className="px-2 py-4 text-center w-16 border-l border-gray-600">
            <Link
              to={`/admin/other/${item.id}`}
            ...
            <tbody>
              {items.map((item, index) => (
                ...
                >
                  {isMonsterType ? (
                    <>
                      <td className="p-4 border-r border-gray-600 text-orange-400 font-bold text-lg text-center">
                        {item.name}
                      </td>
                      ...
                      <td className="p-4 border-r border-gray-600 text-purple-400 text-sm text-center">
                        {item.abilities || "Nenhuma habilidade especial"}
                      </td>
                      <td className="p-4 text-yellow-400 text-sm text-center">
                        {item.loot || "Nenhum loot"}
                      </td>
                    ...
                    <>
                      ...
                      <td className="p-4 border-r border-gray-600 text-gray-300 font-semibold text-center">
                        <span className="bg-gray-900/30 px-2 py-1 rounded">
                          {item.weight || "-"}
                        </span>
                      </td>
                      <td className="p-4 border-r border-gray-600 text-red-400 text-sm text-center">
                        {item.dropBy || "Não informado"}
                      </td>
                      <td className="p-4 text-green-400 text-sm text-center">
                        {item.sellTo || "Não vendável"}
                      </td>
                      {renderEditButton(item)}
                    </>
                  ) : isBookType ? (
                    <>
                      ...
                      <td className="p-4 border-r border-gray-600 text-gray-300 text-sm text-center">
                    ...
                    <>
                      ...
                      <td className="p-4 text-purple-400 text-sm text-center">
                        {item.buffs || "Nenhum buff"}
                      </td>
                      {renderEditButton(item)}
                    </>
                  ) : isRecipeType ? (
                    <>
                      ...
                      <td className="p-4 border-r border-gray-600 text-yellow-400 text-sm text-center">
                        {item.requirements || "Sem requisitos"}
                      </td>
                      <td className="p-4 text-red-400 text-sm text-center">
                        {item.dropBy || "Não informado"}
                      </td>
                      {renderEditButton(item)}
                    </>
                    ...
                    <>
                      ...
                      <td className="p-4 text-green-300 text-center">
                        {item.sellTo || "-"}
                      </td>
                      {renderEditButton(item)}
                    </>
