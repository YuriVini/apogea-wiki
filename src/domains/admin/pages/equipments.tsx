...
export const Equipments = () => {
  ...
  const groupedEquipments = equipments.reduce((acc, equipment) => {
    ...
    return acc
  }, {} as Record<string, EquipmentsApiTypes.Equipment[]>)
        ...
        <div className='max-w-7xl mx-auto'>
            ...
            <div className='bg-gray-800/50 p-4 rounded-lg'>
              <div className='grid grid-cols-1 md:grid-cols-3 gap-4 text-center'>
                ...
                <div className='bg-green-900/30 p-4 rounded-lg'>
                  <div className='text-2xl font-bold text-green-400'>{equipments.filter((eq) => eq.type === 'weapon').length}</div>
          ...
          <div className='space-y-8'>
            {Object.entries(groupedEquipments).map(([category, categoryEquipments]) => (
              <WeaponTable key={category} title={`${category.charAt(0).toUpperCase() + category.slice(1)} (${categoryEquipments.length})`} weapons={categoryEquipments} />
            ))}
          </div>
